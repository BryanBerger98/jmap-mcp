...
| Decoded bytes per `mail_attachment_fetch` | 8000, `maxBytes` from 200 to `files.maxDownloadSize` per call | `src/domains/mail/attachment-fetch.ts` | The output is cut and says so |
